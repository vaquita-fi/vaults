...
# Enhanced Deploy to Base Mainnet script
echo "🚀 Preparing to deploy vault to Base Mainnet..."
...
while [[ "$#" -gt 0 ]]; do
  case $1 in
    ...
    --help) 
      echo "Usage: $0 [--no-verify] [--token USDC|WETH|cbBTC]"
      echo "  --no-verify: Skip contract verification"
      echo "  --token: Token type to deploy (USDC, WETH, cbBTC)"
      exit 0 ;;
...
# Token type to deploy (USDC, WETH, cbBTC)
# TOKEN_TYPE=USDC
...
# Validate token type
if [[ "$TOKEN_TYPE" != "USDC" && "$TOKEN_TYPE" != "WETH" && "$TOKEN_TYPE" != "cbBTC" ]]; then
  echo "❌ Invalid token type: $TOKEN_TYPE. Must be USDC, WETH, or cbBTC."
  exit 1
fi
...
echo "🚀 Running enhanced deployment script on Base Mainnet..."
echo "📊 Token: $TOKEN_TYPE"
echo "🔍 Verification: $VERIFY"
...
echo "   npx hardhat run scripts/interact-with-vault.js --network base"
echo ""
